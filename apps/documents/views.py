...
from apps.documents.services.doc_upload_services import DocumentUploadService



class DocIngestionView(APIView):
     """Handle document upload and ingestion.

        Args:
            request: DRF Request object containing uploaded file and metadata.

        Returns:
            Response: JSON response with upload status and document details."""
    ...
    def post(self, request):
        ...
        result = DocumentUploadService.ingest_document(**data)
        return Response({
            "status": True,
